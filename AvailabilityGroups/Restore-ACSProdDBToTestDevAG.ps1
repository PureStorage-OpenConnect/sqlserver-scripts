<#
Restore-ACSProdDBToTestDevAG.ps1

: Revision 1.0.0.0
:: initial release

Example script restore the latest SQL Server database that was backed up via an application consistent snapshot using the Pure Storage Backup SSMS extension and add it to an Availability Group on a a Test/Dev server, refreshing the database with th elatest production data.
...
Requirements:
  ...
  This script requires the Pure Storage SSMS Extension be installed and the DBATools module be imported.
  ...
  This script must be run from a SQL management machine with SSMS and the PureStorage SSMS extension installed.
  PowerShell Remoting must be enabled on the target mount computer.

This script is provided without warranty and should not be used in a production environment without proper modifications.

.SYNOPSIS
  This script is will restore the latest SQL Server database that was backed up via an application consistent snapshot using the Pure Storage Backup SSMS extension and add it to an Availability Group on a a Test/Dev server, refreshing the database with th elatest production data.
